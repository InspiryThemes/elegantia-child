...
if ( ! function_exists( 'inspiry_enqueue_child_styles' ) ) {
	...
	function inspiry_enqueue_child_styles() {
		...
		if ( ! is_admin() ) {
			...
			// child custom css
			wp_enqueue_style( 'child-custom', get_stylesheet_directory_uri() . '/css/child-custom.css', array( 'parent-default' ), '1.0', 'all' );
...
if ( ! function_exists( 'inspiry_elegantia_child_theme_setup' ) ) {
	/**
